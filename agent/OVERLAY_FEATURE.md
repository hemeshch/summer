# StellAgent Fullscreen Overlay Feature
...
- **show_overlay** - Display fullscreen overlay with custom text
  - `title` (optional): Main text (default: "StellAgent")
...
- **Enable/Disable**: Set `STELLAGENT_SHOW_OVERLAY=false` to disable the overlay
...
3. The overlay shows either:
   ...
   - "StellAgent is working..." text (fallback)
...
```bash
# Disable overlay
export STELLAGENT_SHOW_OVERLAY=false

# Enable overlay (default)
export STELLAGENT_SHOW_OVERLAY=true
```

### Custom Icon
Place your `icon.png` in one of these locations:
- Current working directory: `./icon.png`
- Assets folder: `stellagent_client/assets/icon.png`
- Project root: `/path/to/stellagent-client/icon.png`
...
1. **No overlay appears**: Check if `STELLAGENT_SHOW_OVERLAY` is set to `false`
