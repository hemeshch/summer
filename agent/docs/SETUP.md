...
3. **Run the Client**
   ```bash
   python -m stellagent_client.main run
   ```
...
1. Your `.env` file exists in the stellagent-client directory
...
```bash
python -m stellagent_client.main status
```
...
2. **Command Line Argument**:
   ```bash
   python -m stellagent_client.main run --api-key 'your-key-here'
   ```
...
```bash
python -m stellagent_client.main reset-persistence
```

Or reset when starting:
```bash
python -m stellagent_client.main run --reset-state
```