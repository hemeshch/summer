...
```bash
python -m stellagent_client.main debug
```

Options:
- `--port PORT`: Specify the port (default: 8069)
- `--runtime-dir DIR`: Specify runtime directory (default: stellagent_runtime)

Example with custom port:
```bash
python -m stellagent_client.main debug --port 8080
```
...
1. Create your tool provider in `stellagent_client/tools/`
