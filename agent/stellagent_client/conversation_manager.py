    ...
    def handle_prompt(self, message: Dict[str, Any]):
        ...
        try:
            ...
            def tool_callback(tool_id: str, parameters: Dict[str, Any]) -> tuple:
                print(f"Executing tool: {tool_id} with parameters: {parameters}")
                ...
                # Note: Broadcasting now happens early when tool starts streaming
                # No need to broadcast again here since it was already done
            ...
            # Create early broadcast callback for streaming tool calls
            def early_tool_broadcast(tool_name: str):
                """Broadcast tool call immediately when it starts streaming"""
                print(f"🚀 Tool starting: {tool_name}")
                try:
                    # Get display name if available
                    display_name = tool_name
                    if tool_name in self.tool_manager.tools:
                        display_name = self.tool_manager.tools[tool_name].get_display_name()

                    # Broadcast with minimal info (no parameters yet)
                    self.websocket_handler.broadcast_tool_call(
                        conversation_id,
                        tool_name,
                        display_name,
                        {"status": "streaming"}  # Indicate parameters are still streaming
                    )
                except Exception as e:
                    print(f"Failed to early broadcast tool call: {e}")
            ...
            response, new_history = self.claude_agent.process_prompt(
                ...
                working_directory=conversation.working_directory,
                early_tool_broadcast=early_tool_broadcast
            )
