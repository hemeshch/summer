...
class ClaudeAgent:
                ...
                if facts_file.exists():
                    try:
                        with open(facts_file, "r", encoding="utf-8") as f:
                            ...
                            if facts:
                                user_facts_section = "\n\nStella has remembered the following facts about the user:\n"
                                for fact in facts:
    ...
    def _make_api_call_with_retry(self, messages, tools, extra_headers=None, early_tool_broadcast=None):
                        ...
                        if event.type == 'content_block_start':
                            if hasattr(event, 'content_block'):
                                if event.content_block.type == 'text':
                                    ...
                                    # Broadcast early that a tool is being called (before parameters stream in)
                                    if early_tool_broadcast and tool_block.name:
                                        try:
                                            early_tool_broadcast(tool_block.name)
                                        except Exception as e:
                                            print(f"Failed to broadcast early tool call: {e}")
    ...
    def process_prompt(
        ...
        early_tool_broadcast: Optional[callable] = None
        ...
        Args:
            ...
            early_tool_broadcast: Callback for early tool call notifications (called when tool starts streaming)
        """
        ...
        try:
            while True:
                ...
                content_blocks = self._make_api_call_with_retry(new_history, all_tools, extra_headers, early_tool_broadcast)
