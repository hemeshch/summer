...
class ShortcutsToolProvider(BaseToolSetProvider):
    ...
    def __init__(self, websocket_handler=None):
        ...
        env_base = os.environ.get("SUMMER_SHORTCUTS_DIR")
        if env_base:
            self.shortcuts_base = Path(env_base).expanduser()
        else:
            self.shortcuts_base = Path(__file__).resolve().parents[3] / "shortcut-tools"

        # Ensure the shortcuts directory exists
        if not self.shortcuts_base.exists():
            raise RuntimeError(
                f"Shortcuts tools directory not found at {self.shortcuts_base}. "
                "Set SUMMER_SHORTCUTS_DIR to point at your shortcut-tools directory."
            )
