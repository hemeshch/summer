...
A sophisticated AI agent client that connects to the StellAgent server via WebSocket, processes natural language prompts using Claude API, and provides an extensible plugin system for custom tools. This client acts as an intelligent assistant capable of performing various tasks through its modular tool architecture while maintaining conversation context and persistent state.

## Features

- 🔌 **Real-time WebSocket Connection**: Maintains persistent connection to StellAgent server for instant communication
...
The StellAgent Client follows a modular architecture with clear separation of concerns:
...
```bash
# Basic usage
stellagent-client run

# With options
stellagent-client run --reset-state              # Clear all persisted state
stellagent-client run --runtime-dir ./custom_dir  # Use custom runtime directory
stellagent-client run --model claude-3-opus-20240229  # Use different Claude model
```
...
|---------|-------------|---------|
| `run` | Start the client and connect to server | `stellagent-client run` |
| `status` | Check client status and conversations | `stellagent-client status` |
| `runtime-info` | Display runtime directory structure | `stellagent-client runtime-info` |
| `reset-persistence` | Reset all persistence data | `stellagent-client reset-persistence --force` |
| `debug` | Launch web-based debug interface | `stellagent-client debug --port 8069` |
| `test-prompt` | Test prompt processing locally | `stellagent-client test-prompt "Hello!"` |

### Command Options

#### `run` Command Options
- `--reset-state`: Clear all persistence data before starting
- `--runtime-dir PATH`: Specify custom runtime directory (default: `stellagent_runtime`)
...
stellagent-client run --reset-state

# Use a development environment
stellagent-client run --runtime-dir ./dev_runtime --model claude-3-haiku-20240307

# Check what conversations are active
stellagent-client runtime-info

# Test a complex prompt with tools
stellagent-client test-prompt "List files in the current directory and calculate 15% tip on $85.50"

# Clean up after testing
stellagent-client reset-persistence --force

# Launch debug interface for local tool testing
stellagent-client debug --port 8069
```
...
Create a new Python file in the `stellagent_client/tools/` directory:

```bash
touch stellagent_client/tools/my_custom_tool.py
```
...
- **Purpose**: Maintains persistent connection to StellAgent server
- **Features**:
...
```mermaid
sequenceDiagram
    participant Server as StellAgent Server
...
working_dir = per_conversation_state.get('_working_directory')
# Example: /path/to/stellagent_runtime/conversations/conv_abc123
...
| `DEFAULT_RUNTIME_DIR` | Runtime directory path | `stellagent_runtime` |
...
- Clear conversation: `stellagent-client run --reset-state`
...
- Verify tool file is in `stellagent_client/tools/`
...
- Review loaded tools: `stellagent-client status`
...
rm stellagent_runtime/tool_states.pkl
```
...
python -u stellagent_client/main.py run 2>&1 | tee debug.log
```
...
The StellAgent Client includes a web-based debug interface for testing and developing tools locally without connecting to the WebSocket server. This provides a convenient way to test your tools in isolation.

### Starting the Debug Server

```bash
# Start with default settings (port 8069)
stellagent-client debug

# Use custom port
stellagent-client debug --port 8080

# Use custom runtime directory
stellagent-client debug --runtime-dir ./test_runtime

# Or run directly with Python
python -m stellagent_client.main debug --port 8069
```
...
1. **Create your tool** in `stellagent_client/tools/`
2. **Start the debug server**: `stellagent-client debug`
...
$ stellagent-client debug
Initializing StellAgent Client...
Loading tools...
...
**Tools not loading:**
- Check tools are in `stellagent_client/tools/`
...
git clone https://github.com/yourusername/stellagent-client.git
cd stellagent-client
...
# Run with coverage
pytest --cov=stellagent_client --cov-report=html

# Test individual tools
stellagent-client test-prompt "Calculate 15% tip on $50"
```
...
black stellagent_client/

# Check style with flake8
flake8 stellagent_client/

# Type checking with mypy
mypy stellagent_client/
```
...
```python
import pytest
from stellagent_client.tools.my_tool import MyToolProvider
...
- Report issues: [GitHub Issues](https://github.com/yourusername/stellagent-client/issues)
- Contribute: See [Contributing Guidelines](#contributing-guidelines)
- Discord: Join our developer community

## Summary

The StellAgent Client provides a robust, extensible platform for building AI-powered agents that can:

- **Connect**: Maintain real-time WebSocket connections to the StellAgent server
...
Whether you're building simple automation tools or complex AI workflows, the StellAgent Client provides the foundation for creating intelligent, context-aware agents that can interact with various systems and APIs while maintaining conversation context and state.
